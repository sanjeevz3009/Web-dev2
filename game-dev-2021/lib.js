...
const pointer = {
  ...
  degrees: 0, // angle of xy point relative to screen centre in degrees
...
function mouseMoved(e) {
  ...
  let degrees = pointer.angle * 180/ Math.PI;
...
function drawPointerPos() {
  const radius = pointer.radius / step * 50;
  context.beginPath();
...
function drawUserPos() {
  ...
  context.beginPath();
  context.moveTo(halfWidth, halfHeight);
  context.arc(halfWidth, halfHeight, radius, 0, TAU);
  context.fillStyle = colours[colIndex];
  context.fill();
}
